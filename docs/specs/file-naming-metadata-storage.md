...
**Left open, to be settled as part of implementing this:**
- The exact field set per content kind — does film gain a studio/production-company field (it has none today)? Does scene gain a title/scene-name component (it has none today — scene is currently `Studio/Studio - Performers - YYYY-MM-DD.ext`)?
- **Round-trip recovery.** Writing a field into the filename only satisfies D11b's purpose if it can also be parsed back out on reimport. The current code (`d676b69`, adding performers to film/personal/episodic) writes performers into the filename but `MediaTitleParser.swift` has no concept of performers — it cannot read them back out. Until that's built, D11b's stated guarantee does not actually hold.
- Truncation hierarchy: whether the never-dropped field set (previously just the year, per D11/D12 and issue #91's fix) needs to expand now that performers are canonical, filename-carried metadata.
Tracked in GitHub: [mranlett/ViLM#94](https://github.com/mranlett/ViLM/pull/94) (code, incomplete against this decision) and the next sprint's Track A.
