...
## D11b — superseding D11a, 2026-09-15
D11a held that performer credits are canonical metadata but a filename is a deliberately lossy projection, and kept film/episodic/personal filenames performer-free. The operator has since made the opposite call, for a reason D11a didn't weigh: **the filename is the disaster-recovery layer.** If the ViLM database is lost, the filename is the only metadata that survives — so anything that must survive DB loss has to live there, not just in the catalogue or the sidecar.
**Under D11b:** performer names, studio names, scene names, and season/episode numbers persist in the filename for every content kind where that field applies.
This directly overrides the "naked file resilience" tradeoff that arc #89's synthesis (Gemini Finding 3 / [Metadata Sidecars — portability, not recovery](https://app.notion.com/p/3baadccaf42881c19cc8f7d112c571e5)) had accepted as acceptable — under the operator's standard, it isn't.
**Left open, to be settled as part of implementing this — ✅ ALL THREE RESOLVED 2026-09-16:**
- ✅ **DECIDED 2026-09-16 — yes, film gains a studio segment.** `Title - Studio - Cast (YYYY)`, studio placed between title and cast. Resolved from the matched studio lexicon — the same `StudioPlacement` disposition `scene()` already used, never the raw `studio:` tag string. An unmatched or unprocessed studio has no name to write, so the segment is simply absent rather than invented. (Scene's own title/scene-name component was already answered earlier, under "The scene grammar gains the title, 2026-08-15" — that half of this question was stale by the time D11b was written.)
- ✅ **DECIDED 2026-09-16 — round-trip recovery is built.** `recoverFilmOrPersonalName` and `recoverEpisodicName` in `ContentNaming.swift` parse performers (and now studio) back out of a generated name, syntactically, with no database and no roster required. A single surviving optional segment is a genuine ambiguity (could be studio or cast, or cast or title) and is reported as `ambiguousSegment` rather than guessed — resolvable with `knownStudioNames`/`knownPerformerNames` when a roster is available.
- ✅ **DECIDED 2026-09-16 — cast gets a third tier, not an expansion of "never-dropped."** The never-dropped set stays exactly what it was (year for film/personal, date for scene, series + `SxxEyy` for episodic) — it does NOT expand. Instead cast gets its own new tier: droppable down to a **guaranteed minimum of one** credited performer if any were credited at all, across every grammar that carries it (film/personal, scene, episodic). Studio (film's new field) has no such guarantee — it is fully droppable, and dropped first, before the title is even shortened, as the newest and least essential field.
Tracked in GitHub: [mranlett/ViLM#94](https://github.com/mranlett/ViLM/pull/94) (all three now implemented against this decision) and the next sprint's Track A.
